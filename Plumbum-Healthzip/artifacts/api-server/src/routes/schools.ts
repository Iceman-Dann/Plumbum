...
// scores each using the same deterministic risk function as /api/risk.
import { Router } from "express";

const router = Router();

// ---------------------------------------------------------------------------
// Re-use the same scoring logic from risk.ts (kept in sync by being copied)
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    const scored = await Promise.all(
      top.map(async (place) => {
        ...
        const score = computeRiskScore(fips);
